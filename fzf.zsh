# FZF
export FZF_DEFAULT_COMMAND="fd --type f --strip-cwd-prefix --hidden --follow --exclude .git --preview 'bat --style=numbers --color=always --line-range :500 {}'"
# export FZF_DEFAULT_COMMAND="fd --type f --color=never"
export FZF_DEFAULT_OPTS='--height 64% --layout=reverse --border --color=fg:#d0d0d0,bg:#2e2e2e,hl:#5f87af --color=fg+:#d0d0d0,bg+:#2e2e2e,hl+:#648ce3 --color=info:#6d7d85,prompt:#b53c10,pointer:#5895db --color=marker:#87ff00,spinner:#2b751c,header:#87afaf'
# export FZF_DEFAULT_OPTS="--extended"
export FZF_CTRL_T_OPTS="--preview 'bat --style=numbers --color=always --line-range :500 {}'"

export FZF_ALT_C_COMMAND="fd --type d . --color=never"
export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND $FZF_CTRL_T_OPTS"
...
fzf_completion() {
    ...
    eval "$(
        ...
        local full_functions="$(functions + | grep -Fvx "$(functions -u +)")"
            ...
            (
                ...
                stderr="$(
                    _fzf_completion_preexit() {
                        ...
                        functions + | grep -Fvx -e "$(functions -u +)" -e "$full_functions" | while read -r f; do which -- "$f"; done >&"${__evaled}"
                    }
